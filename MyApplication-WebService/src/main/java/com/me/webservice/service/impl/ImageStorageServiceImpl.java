...
import com.me.common.mapper.ImageStorageMapper;
...
public class ImageStorageServiceImpl implements ImageStorageService {
	...
	@Autowired
	private ImageStorageMapper storageMapper;
	
	@Override
	public ImageStorageDto save(MultipartFile image, ImageEnum imageEnum) throws CustomException {
		try {
			...
			if (imageEnum == ImageEnum.BRAND) {
				...
				imageName = ImageEnum.BRAND + "_" + LocalDateTime.now().format(formatter);
				...
				imageName = ImageEnum.PRODUCT + "_" + LocalDateTime.now().format(formatter);
				imagePath = path.resolve(brandPath).resolve(imageName);
			} else {
			...
			storage.setLocalPath(imagePath + "/" + imageName);
			return storageMapper.entityToDto(storageRepository.save(storage));
		}
